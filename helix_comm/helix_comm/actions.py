...
def do_home(node, args, rate):
    ...
    _publish_loop(
        ...
        'home (nominal tendon lengths)', rate, duration=hold)
...
def do_circle(node, args, rate):
    ...
    period = float(args[4]) if len(args) > 4 else 16.0   # seconds per lap (robot is slow)
...
def do_tendon_demo(node, args, rate):
    ...
    _publish_loop(
        node,
        lambda: node.arm.set_tendon_lengths(tendon_names, nominal),
        'default position (nominal tendon lengths)', rate,
    ...
    for name, base in zip(tendon_names, nominal):
        ...
        _publish_loop(
            ...
            f'{name} = {base - amplitude:.3f}', rate, duration=hold,
            quiet=True)
        if not rclpy.ok():
            break
        node.get_logger().info(
        ...
        _publish_loop(
            ...
            f'{name} = {base:.3f}', rate, duration=hold,
            quiet=True)
        if not rclpy.ok():
            break
    if rclpy.ok():
