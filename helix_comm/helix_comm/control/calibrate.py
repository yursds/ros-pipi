...
class Calibrate:
    ...
    def status(self):
        ...
        for dxl_id in range(9):
            try:
                ...
                self.logger.info(
                    f"  motor_{dxl_id}/current_limit = {v['double_value']} mA"
                )
    ...
    def set_current(self, milliamps):
        ok = self._set_param(
            "/helix/tendon_calibration_node", "calibration_current_ma", milliamps
        )
        if ok:
    ...
    def start(self):
        svc = roslibpy.Service(
            self.client, "/helix/start_calibration", "std_srvs/srv/Empty"
        )
    ...
    def finish(self):
        svc = roslibpy.Service(
            self.client, "/helix/finish_calibration", "std_srvs/srv/Empty"
        )
        res = svc.call(roslibpy.ServiceRequest({}))
        self.logger.info(f"  {res}")
