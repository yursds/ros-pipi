...
class Gripper:
    ...
    def open(self):
        svc = roslibpy.Service(
            self.client, "/helix/gripper/open", "std_srvs/srv/Trigger"
        )
    ...
    def close(self):
        svc = roslibpy.Service(
            self.client, "/helix/gripper/close", "std_srvs/srv/Trigger"
        )
        res = svc.call(roslibpy.ServiceRequest({}))
    ...
    def set_position(self, position):
        svc = roslibpy.Service(
            self.client,
            "/helix/gripper/set_position",
            "helix_interfaces/srv/SetFloat32",
        )
